...
class DenoiseST():
    ...
    def train(self):
        ...
        for epoch in tqdm(range(self.epochs)):
            ...
            self.hiden_feat, self.emb, ret, ret_a, zinb_loss, meanbatch, dispbatch , pibatch= self.model(self.features, self.features_a, self.adj)
            if epoch==0:
                copyemb=self.emb
            ...
            loss = self.alpha * self.loss_feat + 0.5*zinb_loss
        ...
        with torch.no_grad():
            ...
            if self.deconvolution:
                ...
                self.adata.obsm['emb'] = self.emb.detach().cpu().numpy()
                return self.adata
    ...
    def train_map(self):
        ...
        self.optimizer_map = torch.optim.Adam(self.model_map.parameters(), lr=self.learning_rate,
                                              weight_decay=self.weight_decay)
